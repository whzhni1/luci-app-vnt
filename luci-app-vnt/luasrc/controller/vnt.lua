...
function index()
	...
	entry({"admin", "vpn", "vnt", "restart_client"}, call("restart_client")).leaf = true
	entry({"admin", "vpn", "vnt", "restart_server"}, call("restart_server")).leaf = true
...
function act_status()
	...
	local function get_runtime(file)
		...
		if f then
			...
			if start then
				...
				local d, h, m, s = math.floor(diff/86400), math.floor((diff%86400)/3600), math.floor((diff%3600)/60), diff%60
				return d > 0 and string.format("%d天%02d时%02d分", d, h, m) or string.format("%02d时%02d分%02d秒", h, m, s)
			end
		end
		return "-"
	end
	...
	if e.srunning then
		...
		if e.vntsram == "" then e.vntsram = "-" end
	else
		e.vntscpu, e.vntsram = "-", "-"
	end
...
function save_client()
	...
	if data then
		...
		local cfg = json.parse(data)
		if cfg then
			local lists = {localadd=1, peeradd=1, vntdns=1, stunhost=1, mapping=1, checkip=1}
			for k, v in pairs(cfg) do
				if lists[k] then
					uci:delete("vnt", section, k)
					if type(v) == "table" then
						for _, item in ipairs(v) do
							if item ~= "" then uci:add_list("vnt", section, k, item) end
						end
					elseif v ~= "" then
						uci:add_list("vnt", section, k, v)
					end
				else
...
function get_ifaces()
	...
	for iface in ifaces:gmatch("%S+") do
		local ip = sys.exec("ip -4 addr show " .. iface .. " 2>/dev/null | grep inet | awk '{print $2}' | cut -d'/' -f1"):gsub("%s+", "")
		if ip ~= "" then
			table.insert(result, {name = iface, ip = ip})
		end
	end
	luci.http.prepare_content("application/json")
	...
	http.setfilehandler(function(meta, chunk, eof)
		...
		if eof and fp then
			...
			if file:match("%.tar%.gz$") then
				os.execute("tar -xzf /tmp/" .. file .. " -C /tmp/")
			end
...
function get_log()
	...
	luci.http.write(f and f:read("*all") or "暂无日志")
...
function vnt_info()
	...
	info = info:gsub("Connection status", "连接状态"):gsub("Virtual ip", "虚拟IP"):gsub("Virtual gateway", "虚拟网关")
	info = info:gsub("Virtual netmask", "虚拟掩码"):gsub("NAT type", "NAT类型"):gsub("Relay server", "服务器")
